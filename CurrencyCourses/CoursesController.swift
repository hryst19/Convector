    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: "startLoadingXML"), object: nil, queue: nil) { (notification) in
            ...
            DispatchQueue.main.async {
               let acivityIndicator = UIActivityIndicatorView(style: UIActivityIndicatorView.Style.gray)
                acivityIndicator.color = UIColor.red
                acivityIndicator.startAnimating()
                
                self.navigationItem.rightBarButtonItem?.customView = acivityIndicator
            }
        }
...
//       cell.detailTextLabel?.text = courseForCell.Rate
